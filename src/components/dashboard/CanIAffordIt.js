...
export function CanIAffordIt({ availableBalance, fixedExpenses, budgets, goals, onClose }) {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '1.25rem' }}>
          <div>
            ...
            <input type="number" inputMode="decimal" placeholder="0.00" value={amount}
              ...
              style={{ ...S.inp, fontSize: '1.5rem', fontWeight: 800, textAlign: 'center', color: '#f1f5f9' }} />
          </div>
          ...
          <div>
            <label style={S.label}>What is it? (optional)</label>
            <input type="text" placeholder="e.g. New shoes, dinner, etc." value={desc}
              onChange={(e) => setDesc(e.target.value)} style={S.inp} />
          </div>
        </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              ...
              <div style={{ background: '#0f172a', borderRadius: 12, padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {[
                  ...
                  { label: 'Available balance',        value: formatUSD(result.safe),         color: '#f1f5f9' },
                  ...
                  result.budgetLeft !== null && { label: `${category} budget left`, value: formatUSD(result.budgetLeft), color: result.exceedsBudget ? '#f43f5e' : '#22c55e' },
                ...
                <div style={{ background: 'rgba(245,158,11,.08)', border: '1px solid rgba(245,158,11,.2)', borderRadius: 10, padding: '0.875rem' }}>
                  ...
                  {result.goalImpacts.map((g) => (
                    <p key={g.name} style={{ fontSize: '0.85rem', color: '#94a3b8' }}>
                      🎯 <strong style={{ color: '#f1f5f9' }}>{g.name}</strong> delayed by ~{g.delayDays} day{g.delayDays !== 1 ? 's' : ''}
                    </p>
                  ))}
                </div>
              )}
