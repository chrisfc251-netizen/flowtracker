/**
 * Home.js — v4
 * Fix: wire savings tile to SavingsBreakdownSheet so "Edit Savings by Account" works.
 * FinancialState now accepts accounts/savingsBreakdown/transactions and mounts the sheet.
 */
...
import { SavingsBreakdownSheet } from '../components/dashboard/BalanceSplitCards';
...
function FinancialState({
  ...
  // Savings sheet props
  accounts, savingsBreakdown, transactions,
}) {
    ...
    <>
      <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.625rem' }}>
          ...
          {/* Savings tile — tappable when not in ghost mode */}
          {ghostMode ? (
            <div style={{
              background: '#0d1524', border: '1px solid #131e30',
              borderRadius: 12, padding: '0.875rem',
            ...
            >
              ...
              <p style={{ fontSize: '0.65rem', color: '#475569', lineHeight: 1.4 }}>Tap to see breakdown →</p>
            </button>
          )}
        </div>
      </div>

      {/* Savings breakdown sheet — mounts when savings tile tapped */}
      {showSavingsSheet && (
        <SavingsBreakdownSheet
          ...
          onRefresh={() => setShowSavingsSheet(false)}
        />
      )}
    </>
  );
    ...
    <div style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: 16, padding: '1.125rem', marginBottom: '0.875rem' }}>
      ...
      {!result ? (
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          ...
          />
          <button onClick={check} disabled={!amount} style={{ background: amount ? '#818cf8' : '#1e293b', color: amount ? '#fff' : '#334155', border: 'none', borderRadius: 10, padding: '0.625rem 1rem', fontWeight: 700, fontSize: '0.875rem', cursor: amount ? 'pointer' : 'default', fontFamily: 'inherit' }}>
            Check
          </button>
        </div>
      ) : (
        <>
          ...
          <button onClick={reset} style={{ width: '100%', background: 'transparent', border: '1px solid #334155', borderRadius: 8, color: '#64748b', padding: '0.5rem', fontSize: '0.8rem', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
  ...
  return (
    <div style={{ marginBottom: '0.875rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.625rem' }}>
        ...
        <button onClick={onNavigate} style={{ background: 'transparent', border: 'none', color: '#818cf8', fontSize: '0.75rem', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 3, fontFamily: 'inherit' }}>
...
export default function Home() {
  ...
  const { savingsBreakdown } = computeAccountBalances(transactions, transfers);
  const now = new Date();
